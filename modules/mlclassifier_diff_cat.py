...
class GCNClassifier(nn.Module):
    ...
    def forward(self, img1, img2 = None):
        if img2 is not None:
            ...
            cnn_feats2_reshaped = cnn_feats2.reshape(cnn_feats2.size(0),cnn_feats2.size(1),-1).permute(0,2,1) #16,49,1024
            
            #edit
            cnn_feats = torch.cat((cnn_feats1_reshaped,cnn_feats2_reshaped),dim = 1) #16,96.1024
            # logits = self.fc2(global_states)
            # print('global_states',global_states.shape)
            # print('logits',logits.shape)
            node_states = torch.cat((node_states1, node_states2), dim = 1) #
             
            # print(self.avg_fnt(cnn_feats1).shape)
            ...
            global_states = torch.cat((avg_feats1, avg_feats2), dim = 1) #16,2048
